...
本项目是由 [Websoft9](https://www.websoft9.com) 研发的 **云原生程序** ，大大简化 BigBlueButton 复杂的安装及配置。

## 系统要求

安装本项目，确保符合如下的条件（[详细参考](https://github.com/bigbluebutton/docker#recommended-system-requirements)）：
...
```
sudo wget -N https://raw.githubusercontent.com/Websoft9/StackHub/main/docker-installer.sh; sudo bash docker-installer.sh -r bigbluebutton
...
```
sudo wget -N https://raw.githubusercontent.com/Websoft9/StackHub/main/docker-installer.sh; sudo bash docker-installer.sh -r bigbluebutton -p
```
2.复制package包后安装

将压缩的package包复制到想要安装的服务器或虚拟机，登录 Linux，运行下面的**自动化命令**即可启动安装并显示安装结果。
```
sudo bash install-bigbluebutton
```
...
```
git clone --depth=1 https://github.com/Websoft9/docker-bigbluebutton
...
| 名称 | 端口号 | 用途 |  必要性 |
| --- | --- | --- | --- |
| bigbluebutton-server | 9002 | 浏览器访问 BigBlueButton | Y |
| phpmyadmin | 9090 | 数据库可视化管理工具 | Y |
## 文档

[BigBlueButton 管理员手册](https://support.websoft9.com/docs/bigbluebutton)

## 企业级支持

如果需要企业级支持，请订阅我们提供的 [BigBlueButton 企业级支持版](https://apps.websoft9.com/bigbluebutton) 
